	...
	public function wp_ajax_import_members_from_csv() {
		...
		if ( empty( $results['user_ids'] ) ) {
			...
			wp_send_json_error(
				array(
					'status' => false,
					'message' => sprintf( __( 'Unable to import certain user records: %s', 'pmpro-import-members-from-csv' ), count( $results['errors'] ), implode( ', ', $results['errors']->getMessages() ) ),
				)
			);
			exit;
		} else {
			wp_send_json_success(
				array(
					'status' => true,
					'message' => sprintf( __( 'Imported %s', 'pmpro-import-members-from-csv' ), str_pad( '', count( $results['user_ids'] ), '.' ) . "\n" ),
				)
			);
			exit;
		}
        ...
        */
		...
		if ( ! empty( $this->pmpro_fields['membership_startdate'] ) ) {
			$this->pmpro_fields['membership_startdate'] = date_i18n(
				'Y-m-d 00:00:00',
				strtotime(
					$this->pmpro_fields['membership_startdate'], current_time( 'timestamp' )
				)
			);
		}
		...
		if ( ! empty( $this->pmpro_fields['membership_id'] ) ) {
			...
			( empty( $this->pmpro_fields['membership_enddate'] ) ||
				...
				strtotime( $this->pmpro_fields['membership_enddate'], current_time( 'timestamp' ) ) >= current_time( 'timestamp' ) )
					...
					$wpdb->update(
						$wpdb->pmpro_memberships_users, array(
							'status' => 'active',
						), array(
							'user_id' => $user_id,
							'membership_id' => $this->pmpro_fields['membership_id'],
